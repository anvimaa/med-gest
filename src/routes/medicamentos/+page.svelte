...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
    <div>
      ...
      <p class="text-slate-500">Gira o catálogo de medicamentos disponíveis no sistema.</p>
    </div>
  ...
  <div class="bg-white shadow-sm rounded-xl border border-slate-200 overflow-hidden">
    <div class="p-4 border-b border-slate-200 bg-slate-50">
      <form method="GET" action="/medicamentos" class="flex items-center max-w-sm">
      ...
      <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Princípio Ativo</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Forma / Conc.</th>
            <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Cód. Barras</th>
          ...
          {#each data.medicamentos as medicamento}
            <tr class="hover:bg-slate-50 transition-colors">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-semibold text-slate-900">{medicamento.nome}</div>
                <div class="text-xs text-slate-500 truncate max-w-xs">{medicamento.fabricante || 'Sem fabricante'}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-slate-600">{medicamento.formaFarmaceutica || 'N/A'}</div>
                <div class="text-xs text-slate-400">{medicamento.concentracao || ''}</div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-end space-x-2">
                    <a href="/medicamentos/{medicamento.id}/editar" class="text-indigo-600 hover:text-indigo-900 p-1 rounded-md hover:bg-indigo-50 transition-colors" title="Editar">
                    ...
                    <form method="POST" action="?/delete" use:enhance={() => {
                        ...
                        <button type="submit" class="text-red-600 hover:text-red-900 p-1 rounded-md hover:bg-red-50 transition-colors" title="Apagar" onclick={(e) => { if (!confirm('Tem a certeza que deseja apagar este medicamento?')) e.preventDefault(); }}>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <tr>
              <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                <div class="flex flex-col items-center">
                    ...
                    <p class="text-slate-500 text-lg font-medium">Nenhum medicamento encontrado.</p>
                    {#if search}
