...
<div
  class="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900"
>
  {#if !isAuthPage && user}
    <!-- desktop sidebar -->
    <aside
      class="hidden md:flex w-72 flex-col bg-white border-r border-slate-200 sticky top-0 h-screen z-50"
    >
      ...
      <nav class="grow px-4 pb-4 space-y-2 overflow-y-auto">
        ...
        <SidebarItem href="/lotes" label="Controle de Stock">
          {#snippet icon()}
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-5 w-5"
        ...
        {#if user.role === "admin"}
          <SidebarItem href="/eliminacoes" label="Abates">
            {#snippet icon()}
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5"
        ...
        {#if user.role === "admin"}
          <SidebarItem href="/usuarios" label="Gestão de Usuários">
            {#snippet icon()}
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-5 w-5"
    ...
    <!-- mobile header -->
    <header
      class="md:hidden bg-white border-b border-slate-200 sticky top-0 z-50"
    >
      ...
      {#if mobileMenuOpen}
        <div
          class="absolute inset-x-0 bg-white border-b border-slate-200 shadow-xl py-4 flex flex-col px-4 space-y-2 animate-in slide-in-from-top duration-300"
        >
          <a
            href="/dashboard"
            onclick={toggleMobileMenu}
          ...
          <form method="POST" action="/logout" use:enhance>
            <button
              type="submit"
              class="w-full flex items-center px-4 py-3 rounded-xl text-red-600 font-medium hover:bg-red-50"
              >Sair</button
            >
          </form>
        </div>
