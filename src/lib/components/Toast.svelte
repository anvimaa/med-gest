<script lang="ts">
  import type { Toast } from "$lib/utils/toasts.svelte";
...
<div
  class="flex items-center p-4 min-w-[320px] max-w-md rounded-2xl border shadow-xl {colors[
    item.type
  ]} pointer-events-auto transition-all animate-in slide-in-from-right duration-300"
  role="alert"
>
  <div class="shrink-0">
  ...
  <button
    ...
    class="ml-auto -mx-1.5 -my-1.5 rounded-lg p-1.5 inline-flex items-center justify-center h-8 w-8 hover:bg-black/5 focus:ring-2 focus:ring-black/10 outline-none"
  >
    <svg
